...
// CONSULTA PARA RESUMO - UNINDO VENDAS/ENTRADAS + BOLETOS
$params_resumo = [];
$filtros_resumo = [];
...
$sql_resumo = "
    ...
    LEFT JOIN (
        SELECT 
            v.id_vendedor,
            SUM(CASE WHEN v.forma_pg IN ('PIX', 'Cartão Débito', 'Dinheiro', 'À Vista') THEN 1 ELSE 0 END) AS vendas_avista,
            ...
            SUM(CASE WHEN v.forma_pg IN ('PIX', 'Cartão Débito', 'Dinheiro', 'À Vista') THEN v.vlr_total ELSE 0 END) AS valor_vendas_avista,
            ...
            SUM(CASE WHEN v.forma_pg IN ('PIX', 'Cartão Débito', 'Dinheiro', 'À Vista') THEN (v.vlr_total * COALESCE(c.valor, 0) / 100) ELSE 0 END) AS comissao_vendas_avista,
            SUM(CASE WHEN v.forma_pg IN ('Parcelado', 'Permuta Parcelado', 'Cartão de Crédito') AND v.vlr_entrada > 0 THEN (v.vlr_entrada * COALESCE(c.valor, 0) / 100) ELSE 0 END) AS comissao_entradas,
...
// Unir os parâmetros
$params_final = array_merge($params_resumo, $params_boletos);
...
if ($tipo_relatorio === 'detalhado') {
    ...
    if ($status_filtro === 'avista') {
        $filtro_status_vendas = "AND v.forma_pg IN ('PIX', 'Cartão Débito', 'Dinheiro', 'À Vista')";
    ...
    $sql_detalhado = "
        ...
        UNION ALL
        ($sql_det_boletos)
        ORDER BY data_referencia DESC, nome_vendedor, id_venda, n_parcela
    ";
    
    $stmt_detalhado = $pdo->prepare($sql_detalhado);
    $stmt_detalhado->execute($params_final);
...
<body>
        ...
        <div class="container">
            <div class="row align-items-center">
                <div class="col">
                    ...
                    <p class="text-muted mb-0">Comissões a pagar no período selecionado</p>
                </div>
                <div class="col-auto">
    ...
    <div class="container">
        ...
        <div class="print-header">
            ...
            <p>Sistema de Gestão de Vendas e Comissões</p>
        ...
        <div class="filtros-card no-print">
            ...
            <div class="alert alert-info mb-3">
                <strong>Nova Lógica de Comissão:</strong><br>
                • <strong>Vendas À Vista</strong>: Comissão calculada pela data da venda<br>
                • <strong>Entradas</strong>: Comissão da entrada calculada pela data da venda<br>
                • <strong>Boletos Pagos</strong>: Comissão calculada pela data de vencimento (qualquer venda)
            </div>
        ...
        <?php if ($tipo_relatorio === 'resumo'): ?>
            ...
            <div class="content-card">
                <div class="p-3">
                    <h5 class="mb-3">Resumo por Vendedor</h5>
            ...
            <!-- SEÇÃO DE GERAÇÃO DE RECIBOS - NOVA FUNCIONALIDADE -->
        ...
        <?php else: ?>
            ...
            <div class="content-card">
                <div class="p-3">
                    <h5 class="mb-3">Relatório Detalhado (<?= number_format(count($dados_detalhados)) ?> registros)</h5>
